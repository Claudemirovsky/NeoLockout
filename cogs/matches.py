...
async def plot_graph(ctx, data, handle):
    ...
    embed.set_footer(
        text="Requested by " + str(ctx.author), icon_url=ctx.author.display_avatar.url
    )
...
class Matches(commands.Cog):
    ...
    def make_match_embed(self, ctx):
        desc = (
            "Information about Matches related commands! **[use .match <command>]**\n\n"
        )
        ...
        embed.set_author(
            name="Lockout commands help", icon_url=ctx.me.display_avatar.url
        )
        embed.set_footer(
            text="Use the prefix . before each command. For detailed usage about a particular command, type .help match <command>"
        )
        embed.add_field(
            name="GitHub repository", value=f"[GitHub]({GITHUB_LINK})", inline=True
        )
        embed.add_field(
            name="Bot Invite link", value=f"[Invite]({BOT_INVITE})", inline=True
        )
        embed.add_field(
            name="Support Server", value=f"[Server]({SERVER_INVITE})", inline=True
        )
        return embed

    @commands.group(
        brief="Commands related to matches. Type .match for more details",
    ...
    async def challenge(self, ctx, member: discord.Member, rating: int):
        ...
        if not self.db.get_handle(ctx.guild.id, ctx.author.id):
            await discord_.send_message(
                ctx, "Set your handle first before challenging someone"
            )
            return
        if not self.db.get_handle(ctx.guild.id, member.id):
            await discord_.send_message(
                ctx, f"Handle for your opponent {member.mention} not set"
            )
            return
        if (
            self.db.is_challenging(ctx.guild.id, ctx.author.id)
            or self.db.is_challenged(ctx.guild.id, ctx.author.id)
            or self.db.in_a_match(ctx.guild.id, ctx.author.id)
        ):
            await discord_.send_message(
                ctx,
                "You are already challenging someone/being challenged/in a match. Pls try again later",
            )
            return
        if (
            self.db.is_challenging(ctx.guild.id, member.id)
            or self.db.is_challenged(ctx.guild.id, member.id)
            or self.db.in_a_match(ctx.guild.id, member.id)
        ):
            await discord_.send_message(
                ctx,
                "Your opponent is already challenging someone/being challenged/in a match. Pls try again later",
            )
            return
        if rating not in range(LOWER_RATING, UPPER_RATING - 400 + 1):
            await discord_.send_message(
                ctx,
                f"Invalid Rating Range, enter an integer between {LOWER_RATING}-{UPPER_RATING-400}",
            )
            return
        rating = rating - rating % 100
        resp = await get_time_response(
            self.client,
            ctx,
        ...
        if not resp[0]:
            await ctx.send(
                f"You took too long to decide! Match invalidated {ctx.author.mention}"
            )
            return
        ...
        await ctx.send(
            f"{ctx.author.mention} has challenged {member.mention} to a match with problem ratings from "
            f"{rating} to {rating+400} and lasting {duration} minutes. Type `.match accept` within 60 seconds to accept"
        )
        tme = int(time.time())
        self.db.add_to_challenge(
            ctx.guild.id,
            ctx.author.id,
            member.id,
            rating,
        ...
        if self.db.is_challenging(ctx.guild.id, ctx.author.id, tme):
            await ctx.send(
                f"{ctx.author.mention} your time to challenge {member.mention} has expired."
            )
    ...
    async def accept(self, ctx):
        if not self.db.is_challenged(ctx.guild.id, ctx.author.id):
            ...
            return
        embed = Embed(
            description=f"Preparing to start the match...", color=Color.green()
        )
        embed.set_footer(
        ...
        handle1, handle2 = (
            self.db.get_handle(ctx.guild.id, data.p1_id),
            self.db.get_handle(ctx.guild.id, data.p2_id),
        )
    ...
    async def _invalidate(self, ctx, member: discord.Member):
        ...
        await ctx.send(
            embed=discord.Embed(
                description="Match has been invalidated", color=discord.Color.green()
            )
        )
    ...
    async def invalidate(self, ctx):
        if not self.db.in_a_match(ctx.guild.id, ctx.author.id):
            await discord_.send_message(
                ctx, f"User {ctx.author.mention} is not in a match."
            )
            return
        match_info = self.db.get_match_info(ctx.guild.id, ctx.author.id)
        opponent = await discord_.fetch_member(
            ctx.guild,
            match_info.p1_id if match_info.p1_id != ctx.author.id else match_info.p2_id,
        )
        await ctx.send(
            f"{opponent.mention} you opponent {ctx.author.mention} has proposed to forfeit the match, type `yes` within 30 seconds to accept"
        )
        ...
        try:
            message = await self.client.wait_for(
                "message",
                timeout=30,
                check=lambda message: message.author == opponent
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"{ctx.author.mention} {opponent.mention}, match has been invalidated",
    ...
    async def draw(self, ctx):
        ...
        opponent = await discord_.fetch_member(
            ctx.guild, match.p1_id if match.p1_id != ctx.author.id else match.p2_id
        )
        await ctx.send(
    ...
    async def ongoing(self, ctx):
        ...
        text = "\n".join(
            content[
                currPage * ONGOING_PER_PAGE : min(
                    len(content), (currPage + 1) * ONGOING_PER_PAGE
                )
            ]
        )
    ...
    async def recent(self, ctx, member: discord.Member = None):
        ...
        text = "\n".join(
            content[
                currPage * RECENT_PER_PAGE : min(
                    len(content), (currPage + 1) * RECENT_PER_PAGE
                )
            ]
        )
        embed = discord.Embed(description=text, color=discord.Color.gold())
        ...
        for match in matches:
            try:
                ...
                if resp[1]:
                    ...
                    self.db.add_rating_update(
                        ctx.guild.id, match.p1_id, res[match.p1_id][0]
                    )
                    self.db.add_rating_update(
                        ctx.guild.id, match.p2_id, res[match.p2_id][0]
                    )
                    self.db.delete_match(match.guild, match.p1_id)
                    ...
                    for user in ranklist:
                        pos += (
                            f"{':first_place:' if user[1] == 1 else ':second_place:'}\n"
                        )
                        name += f"{user[0].mention}\n"
                        ratingChange += f"{res[user[0].id][0]} (**{'+' if res[user[0].id][1] >= 0 else ''}{res[user[0].id][1]}**)\n"
                    embed.add_field(name="Position", value=pos)
    ...
    async def rating(self, ctx, member: discord.Member = None):
        ...
        if not self.db.get_handle(ctx.guild.id, member.id):
            await discord_.send_message(
                ctx, f"Handle for user {member.mention} not set"
            )
            return
        if len(data) <= 1:
            await ctx.send(
                embed=discord.Embed(
                    description=f"User {member.mention} is unrated! Compete in matches to become rated"
                )
            )
            return
    ...
    async def ranklist(self, ctx):
        ...
        for x in res:
            try:
                data.append(
                    [(await discord_.fetch_member(ctx.guild, x[0])).name, str(x[1])]
                )
            except Exception:
                pass
        await paginator.Paginator(
            data, ["User", "Rating"], f"Match Ratings", 10
        ).paginate(ctx, self.client)
