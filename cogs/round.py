...
class Round(commands.Cog):
    ...
    def make_round_embed(self, ctx):
        desc = (
            "Information about Matches related commands! **[use .round <command>]**\n\n"
        )
        ...
        embed.add_field(
            name="Support Server",
            value=f"[Server](https://discord.gg/xP2UPUn)",
            inline=True,
        )
        return embed

    @commands.group(
        brief="Commands related to rounds! Type .round for more details",
    ...
    async def challenge(self, ctx, *users: discord.Member):
        ...
        if len(users) == 0:
            await discord_.send_message(
                ctx, f"The correct usage is `.round challenge @user1 @user2...`"
            )
            return
        ...
        tournament = 0
        if len(users) == 2 and (
            await tournament_helper.is_a_match(
        ...
        problems = await codeforces.find_problems(
            [self.db.get_handle(ctx.guild.id, x.id) for x in users] + alts, rating
        )
        if not problems[0]:
        ...
        self.db.add_to_ongoing_round(
            ctx, users, rating, points, problems, duration, repeat, alts, tournament
        )
    ...
    async def ongoing(self, ctx):
        ...
        text = "\n".join(
            content[
                currPage * ROUNDS_PER_PAGE : min(
                    len(content), (currPage + 1) * ROUNDS_PER_PAGE
                )
            ]
        )
    ...
    async def update(self, ctx):
        await ctx.send(
            embed=discord.Embed(
                description="Updating rounds for this server",
        ...
        for round in rounds:
            try:
                ...
                if resp[2] or resp[1]:
                    await channel.send(
                        f"{' '.join([(await discord_.fetch_member(ctx.guild, int(m))).mention for m in round.users.split()])} there is an update in standings"
                    )
                ...
                if resp[1]:
                    ...
                    eloChanges = elo.calculateChanges(
                        [
                            [
                                (await discord_.fetch_member(ctx.guild, user.id)),
                                user.rank,
                                self.db.get_match_rating(round_info.guild, user.id)[-1],
                            ]
                    ...
                    for id in list(map(int, round_info.users.split())):
                        self.db.add_rating_update(
                            round_info.guild, id, eloChanges[id][0]
                        )
                    ...
                    if round_info.tournament == 1:
                        ...
                        if ranklist[1].rank == 1 and tournament_info.type != 2:
                            ...
                            if not res[0]:
                                ...
                                match_resp = await self.api.post_match_results(
                                    res[1]["tournament_id"],
                                    res[1]["match_id"],
                                    scores,
                                ...
                                if not match_resp or "errors" in match_resp:
                                    ...
                                    continue
                                winner_handle = self.db.get_handle(
                                ...
                                if await tournament_helper.validate_tournament_completion(
                                    round_info.guild, self.api, self.db
                                ):
                                    await self.api.finish_tournament(
                                        res[1]["tournament_id"]
                                    )
                                    await asyncio.sleep(3)
                                    winner_handle = await tournament_helper.get_winner(
                                        res[1]["tournament_id"], self.api
                                    )
                                    await channel.send(
                                        embed=tournament_helper.tournament_over_embed(
                                            round_info.guild, winner_handle, self.db
                                        )
                                    )
                                    self.db.add_to_finished_tournaments(
                                        self.db.get_tournament_info(round_info.guild),
                                        winner_handle,
                                    )
    ...
    async def custom(self, ctx, *users: discord.Member):
        ...
        if len(users) == 0:
            await discord_.send_message(
                ctx, f"The correct usage is `.round custom @user1 @user2...`"
            )
            return
        if ctx.author not in users:
            users.append(ctx.author)
        ...
        problems = await discord_.get_problems_response(
            self.client,
            ctx,
        ...
        self.db.add_to_ongoing_round(
            ctx, users, rating, points, problems, duration, 0, [], tournament
        )
        round_info = self.db.get_round_info(ctx.guild.id, users[0].id)
