...
class Handles(commands.Cog):
    ...
    def make_handle_embed(self, ctx):
        desc = (
            "Information about Handle related commands! **[use .handle <command>]**\n\n"
        )
    ...
    @commands.group(
        brief="Commands related to handles! Type .handle for more details",
    ...
    async def set(self, ctx, member: discord.Member, handle: str):
        ...
        if len(handles):
            ...
            await discord_.send_message(
                ctx,
                f"{member.mention} Handle {handle} is already in use by {handle_user.mention}",
            )
            return
    ...
    async def remove(self, ctx, member: discord.Member):
        ...
        if not self.db.get_handle(ctx.guild.id, member.id):
            ...
            return
        if self.db.in_a_round(ctx.guild.id, member.id) or self.db.in_a_match(
            ctx.guild.id, member.id
        ):
            await discord_.send_message(
                ctx, f"{member.mention} is currently in a match/round. Try again later"
            )
            return
    ...
    async def identify(self, ctx, handle: str):
        if self.db.get_handle(ctx.guild.id, ctx.author.id):
            await discord_.send_message(
                ctx,
                f"Your handle is already set to {self.db.get_handle(ctx.guild.id, ctx.author.id)}, "
        ...
        if len(handles):
            ...
            await discord_.send_message(
                ctx,
                f"{ctx.author.mention} Handle {handle} is already in use by {handle_user.mention}",
            )
            return
        ...
        if res != await self.cf.get_first_name(handle):
            await discord_.send_message(
                ctx, f"Unable to set handle, please try again {ctx.author.mention}"
            )
            return
    ...
    async def list(self, ctx):
        ...
        for x in data:
            try:
                data1.append(
                    [
                        (await discord_.fetch_member(ctx.guild, int(x[1]))).name,
                        x[2],
                        x[3],
                    ]
                )
        ...
        await paginator.Paginator(
            data, ["User", "Handle", "Rating"], f"Handle List", HANDLES_PER_PAGE
