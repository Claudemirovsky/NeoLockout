...
class Tournament(commands.Cog):
    ...
    @commands.group(
        brief="Commands related to tournaments! Type .tournament for more details",
    ...
    async def setup(self, ctx, tournament_type: int, *, tournament_name: str):
        ...
        **tournament_type:** Integer (0: single elimination, 1: double elimination, 2: swiss)
        ...
        if any([not ch.isalnum() and ch != " " for ch in tournament_name]):
            await discord_.send_message(
                ctx, "The tournament name should contain only alpha-numeric characters"
            )
            return
    ...
    async def register(self, ctx):
        ...
        if not self.db.get_handle(ctx.guild.id, ctx.author.id):
            await discord_.send_message(
                ctx, "Your handle is not set, set your handle first and try again"
            )
            return
        ...
        if ctx.author.id in [x.discord_id for x in registrants]:
            await discord_.send_message(
                ctx, "You have already registered for the tournament"
            )
            return
        if handle_info[2] in [x.handle for x in registrants]:
            await discord_.send_message(
                ctx,
                f"Someone has already registered for the tournament with handle `{handle_info[2]}`",
            )
            return
        if len(registrants) == MAX_REGISTRANTS:
            await discord_.send_message(
                ctx, "The tournament has already reached its max registrants limit!"
            )
            return

        self.db.add_registrant(
            ctx.guild.id, ctx.author.id, handle_info[2], handle_info[3], 0
        )

        await ctx.send(
            embed=discord.Embed(
                description=f"Successfully registered for the tournament. `{MAX_REGISTRANTS-len(registrants)-1}` slots left.",
    ...
    async def unregister(self, ctx):
        ...
        if ctx.author.id not in [x.discord_id for x in registrants]:
            await discord_.send_message(
                ctx, "You have not registered for the tournament"
            )
            return
    ...
    @tournament.command(
        name="_unregister", brief="Forcefully unregister someone from the tournament"
    )
    ...
    @tournament.command(
        name="registrants",
        brief="View the list of users who have registered the tournament",
    )
    async def registrants(self, ctx):
        ...
        await paginator.Paginator(
            [
                [str(i + 1), registrants[i].handle, str(registrants[i].rating)]
                for i in range(len(registrants))
            ],
            ["S No.", "Handle", "Rating"],
            "Registrants for the Lockout tournament",
            15,
    ...
    async def begin(self, ctx):
        ...
        if tournament_info.status == 0:
            ...
            participants_resp = await self.api.bulk_add_participants(
                ...
                [
                    {
                        "name": f"{registrants[i].handle} ({registrants[i].rating})",
                        "seed": i + 1,
                    }
                    for i in range(len(registrants))
                ],
            ...
            desc += (
                f"To play tournament matches just use the `.round` command of the bot and challenge someone to a round. \n"
    ...
    async def matches(self, ctx):
        ...
        await discord_.content_pagination(
            content,
            self.client,
            10,
            ...
            ctx,
            discord.Color.gold(),
            f"View all matches at https://challonge.com/{tournament_info.url}",
        )

    @tournament.command(
        name="forcewin", brief="Grant victory to a user without them competing"
    )
    @commands.cooldown(1, 10, BucketType.user)
    async def forcewin(self, ctx, *, handle: str):
        ...
        for match in matches_resp:
            if match["match"]["state"] != "open":
                continue
            if (
                match["match"]["player1_id"] == challonge_id
                or match["match"]["player2_id"] == challonge_id
            ):
        ...
        scores = await discord_.get_seq_response(
            self.client,
            ctx,
            f"{ctx.author.mention} enter 2 space seperated integers denoting the scores of winner and loser respectively",
            30,
            2,
            ctx.author,
            [0, 10000],
        ...
        resp = await self.api.post_match_results(
            tournament_info.id, match_id, f"{scores[1][0]}-{scores[1][1]}", challonge_id
        )
        if not resp or "errors" in resp:
            ...
            await discord_.send_message(
                ctx, f"Granted victory to user with handle `{handle}`"
            )
    ...
    async def forcedraw(self, ctx, *, handle: str):
        ...
        resp = await self.api.post_match_results(
            tournament_info.id, match_id, f"{scores[1][0]}-{scores[1][1]}", "tie"
        )
        if not resp or "errors" in resp:
            await discord_.send_message(ctx, "Some error occurred, try again later")
            if resp and "errors" in resp:
    ...
    @tournament.command(
        name="match_invalidate",
        brief="Invalidate the results of a match",
    ...
    async def match_invalidate(self, ctx, idx: int):
        ...
        for match in matches_resp:
            if (
                ...
                and match["match"]["suggested_play_order"] == idx
            ):
                match_id = match["match"]["id"]
    ...
    async def recent(self, ctx):
        ...
        for i in range(len(data)):
            content.append(
                f"`{len(data)-i}.` [{data[i].name}](https://challonge.com/{data[i].url}) was won by [{data[i].winner}](https://codeforces.com/profile/{data[i].winner}) "
