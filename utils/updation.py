...
async def update_match(match_info):
    handle1, handle2 = (
        db.get_handle(match_info.guild, match_info.p1_id),
        db.get_handle(match_info.guild, match_info.p2_id),
    )
    enter_time = time.time()
    sub1, sub2 = (
        await cf.get_user_problems(handle1, RECENT_SUBS_LIMIT),
        await cf.get_user_problems(handle2, RECENT_SUBS_LIMIT),
    )
    if not sub1[0]:
        return sub1
    if not sub2[0]:
    ...
    for i in range(5):
        ...
        time1, time2 = (
            codeforces.get_solve_time(
                sub1, int(problems[i].split("/")[0]), problems[i].split("/")[1]
            ),
            codeforces.get_solve_time(
                sub2, int(problems[i].split("/")[0]), problems[i].split("/")[1]
            ),
        )
    ...
    if not judging and (
        enter_time > match_info.time + 60 * match_info.duration
        or no_change_possible(new_status)
    ):
        over = True
    ...
    for user in ranks:
        ...
        res.append(
            User(
                user[2],
                user[0],
                [[x[0], x[1]] for x in ranks].index([user[0], user[1]]) + 1,
            )
        )
    return res
...
async def update_round(round_info):
    ...
    for i in range(len(problems)):
        ...
        times = [
            codeforces.get_solve_time(
                sub, int(problems[i].split("/")[0]), problems[i].split("/")[1]
            )
            for sub in subs
        ]
        ...
        if len(solved) > 0 and round_info.repeat == 1:
            res = await codeforces.find_problems(
                handles + db.fetch_alts(round_info.guild, users[0]), [rating[i]]
            )
            if not res[0]:
                new_problem = "0"
            else:
    ...
    if not judging and (
        ...
        or (
            round_info.repeat == 0
            and no_round_change_possible(status[:], points, problems)
        )
    ):
        over = True
