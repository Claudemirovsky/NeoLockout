...
async def is_a_match(
    guild, p1_id, p2_id, api: challonge_api.ChallongeAPI, db: dbconn.DbConn
):
    ...
    for match in matches:
        ...
        if data["state"] == "open" and (
            (data["player1_id"] == p1_id and data["player2_id"] == p2_id)
            or (data["player1_id"] == p2_id and data["player2_id"] == p1_id)
        ):
            return True
...
async def validate_match(
    ...
    p1_cid, p2_cid = (
        db.get_challonge_id(guild, p1_id),
        db.get_challonge_id(guild, p2_id),
    )
    ...
    for match in matches:
        data = match["match"]
        if data["state"] == "open" and (
            (data["player1_id"] == p1_cid and data["player2_id"] == p2_cid)
            or (data["player1_id"] == p2_cid and data["player2_id"] == p1_cid)
        ):
            res = {
                p1_id: p1_cid,
                p2_id: p2_cid,
...
async def validate_tournament_completion(
    guild, api: challonge_api.ChallongeAPI, db: dbconn.DbConn
):
    tournament_info = db.get_tournament_info(guild)
    if not tournament_info or tournament_info.status != 2:
        return False
