...
class Paginator:
    ...
    def get_page_content(self, page_no):
        ...
        for i in range(len(val)):
            val[i] = max(
                max([len(element[i]) for element in elements]), len(self.headers[i])
            )
        content, dashes = "", ""
    ...
    async def paginate(self, ctx, client):
        embed = Embed(
            title=self.title,
            description=self.get_page_content(self.current_page),
            color=Color(randint(0, 0xFFFFFF)),
        )
        embed.set_footer(
            text="Page %s out of %s" % (str(self.current_page), str(self.total_pages))
        )
