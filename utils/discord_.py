...
def match_problems_embed(match_info):
    ...
    embed = discord.Embed(
        description=f"[{handle1}](https://codeforces.com/profile/{handle1}) (**{a}** points) vs "
    ...
    embed.set_footer(
        text=f"Time left: {timeez((match_info.time + 60 * match_info.duration) - int(time.time()))}"
    )
...
def ongoing_matches_embed(data):
    ...
    for match in data:
        try:
            handle1, handle2 = (
                db.get_handle(match.guild, match.p1_id),
                db.get_handle(match.guild, match.p2_id),
            )
...
def round_problems_embed(round_info):
    ...
    embed.set_footer(
        text=f"Time left: {timeez((round_info.time + 60 * round_info.duration) - int(time.time()))}"
    )
    ...
    for round in data:
        try:
            ranklist = updation.round_score(
                list(map(int, round.users.split())),
                list(map(int, round.status.split())),
...
async def content_pagination(
    content, client, PER_PAGE, heading, ctx, color, extra_text: str = ""
):
    currPage = 0
    ...
    while True:
        try:
            ...
            text = "\n".join(
                content[
                    currPage * PER_PAGE : min(len(content), (currPage + 1) * PER_PAGE)
                ]
            )
            embed = discord.Embed(description=text, color=color)
            embed.set_author(name=heading)
